...
<body>
	...
	while($tupla = mysqli_fetch_assoc($dataset)) {
        ...
        <tr>
            ...
            <td> <?php echo $tupla['idSolicitudProducto']; ?> </td>
			<td> <?php if($tupla['estadoEntrega'] == null ){
				echo "Solicitud sin atender"; }
                else
                {
            ...
            <td><?php if ($_SESSION['category'] == '2' && $_SESSION['departamento'] == '2') { ?></td>
                    <?php if($tupla['estadoEntrega'] == null ){ ?>
                    <td class="actions">
                        <input type="button" value="Entregar" onclick="location.href='../../Data/updateEstadoEnEntrega.php?idSolicitud=<?php echo $tupla['idSolicitud']; ?>'" class="update-btn">
                    </td>
			        <?php } ?>
				    <?php if($tupla['estadoEntrega'] == "En entrega" ) { ?>
				    <td class="actions">
					    <input type="button" value="Entregado" onclick="location.href='../../Data/updateEstadoEntrega.php?idSolicitud=<?php echo $tupla['idSolicitud']; ?>'" class="update-btn">
				    </td>
				    <?php } ?>
		</tr>
                <?php } } } ?>
