...
<body>
	...
	<div class="container">
		...
		<form class="form" method="post" action="../../Data/addRequisicion.php">
		...
		<select name="listOpciones" id=opcionesProducto>
			<?php
			$dataset = $myRequisicion->getAllSuppliesProductos();
		...
		<!-- Codigo para mostrar mensaje guia para conocer al departamento que se está atendiendo-->
		<div id="departamentoTexto"></div>
		<script>
			var productoADepartamento = {
				<?php
				$dataset = $myRequisicion->getAllSuppliesProductos();
				while ($tupla = mysqli_fetch_assoc($dataset)) {
					echo $tupla['idProducto'] . ": '" . $myRequisicion->getNombreDepartamento($myRequisicion->getDepaProducto($tupla['idProducto'])) . "',";
				}
				?>
			};
			document.getElementById('opcionesProducto').addEventListener('change', function() {
				var productoId = this.value;
				var departamento = productoADepartamento[productoId];
				document.getElementById('departamentoTexto').innerText = "Su solicitud será vinculada con el departamento " + departamento;
			});
		</script>
		<br>
		...
		<input placeholder="Unidades del producto solicitado"  type="number" id="cantidad" title="Motivo de la solicitud" name="numCantidad" min = "1" required>
		...
		<a href="../General/generarSolicitudEmpleado.php">
			<div class="go-to-button">Regresar</div>  <br>
		</a>
		</form>
	</div>
	</div>
